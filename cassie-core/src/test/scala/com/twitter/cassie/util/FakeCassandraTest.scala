...
class FakeCassandraTest extends FunSpec with MustMatchers with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  describe("a fake cassandra") {
    ...
    it("should be able to connect to an arbitrary columnfamily and read and write") {
      ...
      Await.result(cf.insert("k", Column("b", "c")))
      Await.result(cf.getRow("k")).size must equal(1)
      Await.result(cf.getColumn("k", "b")).size must equal(1)
    }
    ...
    it("should be able to multiget_slice") {
      ...
      val response = Await.result(cf.multigetColumns(keys, columnNames))
    ...
    it("should be able to get_count") {
      ...
      batch.insert("b", Column("d", "e"))
      Await.result(batch.execute())
      Await.result(cf.getCount("a")) must equal(2)
      Await.result(cf.getCount("c")) must equal(0)
    }
    ...
    it("should be able to multiget_count") {
      ...
      Await.result(cf.multigetCounts(keys)) must equal(counts)
    }
    ...
    it("should be able to batch mutate") {
      ...
      Await.result(batch.execute())
      Await.result(cf.getRow("k")).size() must equal(2)

      batch = cf.batch
      batch.removeColumn("k", "b")
      batch.removeColumn("k", "d")
      Await.result(batch.execute())
      Await.result(cf.getRow("k")).size() must equal(0)
    }

    it("should handle counters") {
      var cf = keyspace().counterColumnFamily("counters", Utf8Codec, Utf8Codec)
      Await.result(cf.add("1", CounterColumn("2", 3)))
      Await.result(cf.getColumn("1", "2")) must equal(Some(CounterColumn("2", 3)))
      Await.result(cf.add("1", CounterColumn("2", 4)))
      Await.result(cf.getColumn("1", "2")) must equal(Some(CounterColumn("2", 7)))
    }
    ...
    it("should support out-of-order inserts") {
      ...
      Await.result(cf.insert("x", Column("y", "z").timestamp(2)))
      Await.result(cf.insert("x", Column("y", "ZZzzz").timestamp(1)))

      Await.result(cf.getColumn("x", "y").map(_.map(_.value must equal("z"))))
    }
    ...
    it("should support super column families") {
      ...
      Await.result(cf.insert("a", "b", Column("c", "d").timestamp(1)))
      val row = Await.result(cf.getRow("a"))
    ...
    it("should support out-of-order inserts to super column families") {
      val cf = keyspace().superColumnFamily[String, String, String, String]("super", Utf8Codec, Utf8Codec, Utf8Codec, Utf8Codec)
      Await.result(cf.insert("a", "b", Column("c", "d").timestamp(2)))
      Await.result(cf.insert("a", "b", Column("c", "D").timestamp(1)))
      val row = Await.result(cf.getRow("a"))
      row.size must equal(1)
    ...
    it("basic row scans") {
      ...
      data foreach { case (k, v) =>
        Await.result(cf.insert(k, Column("columnName", v)))
      }

      // insert a row from a different column name to test that kind of predicate filtering
      Await.result(cf.insert("otherKey", Column("otherColName", "otherValue")))
    ...
    it("should truncate") {
      ...
      // validate its there
      data foreach { case (k, v) =>
        val col = Await.result(cf.getColumn(k, "columnName"), 3.seconds)
